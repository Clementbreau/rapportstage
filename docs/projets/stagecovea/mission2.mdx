...
# Mission 2 — Resynchronisation RGPD des contrats entre DB2 et le Datahub
...
Pour assurer la bonne **conformité** avec les exigences RGPD, il faut donc **resynchroniser** ces données en les réintégrant dans le processus de purge, afin qu’elles soient correctement prises en compte.

---

### Objectif

* Récuperer un Unload de la base DB2 qui correspont aux données n'ayant pas été purgés.
...
`}
  ...
  <p>
    Le fichier data_guide_contrat_incoming sera utilisé par le traitement de purge existant.
  </p>
...
* **Compétence 6** : travail en coordination avec plusieurs équipes (intégrateurs, conformité, exploitation).
